...
- `lib/features/clubs/services/club_service.dart` â€” all club, post, comment, like, pin logic
...
- `lib/features/clubs/screens/clubs_hub_screen.dart` â€” MY CLUBS + DISCOVER tabs, embedded mode
- `lib/features/clubs/screens/club_detail_screen.dart` â€” feed + leaderboard tabs, pin, join/leave/delete
...
- Image picker: image_picker ^1.1.2 â€” used in CreatePostSheet for camera and gallery image selection
...
- Weather card currently shows `Icons.wb_sunny_outlined` as a placeholder icon for all weather types â€” custom painter illustrations planned for polish pass
...
- `ios/Runner/Info.plist` â€” location + motion permission keys, `UIBackgroundModes: location`, `GMSApiKey` placeholder (patched at CI build time), `NSPhotoLibraryAddUsageDescription` for save-to-gallery on share
...
- Share Trip â€” RepaintBoundary + RenderRepaintBoundary.toImage() pipeline captures a 1080Ã—1920 PNG off-screen. Card layout: route polyline (teal, no map base layer) in left column (65% width), four stats stacked vertically in right column (35% width), weather + smoothness side by side below, branding strip (@username + Momentum) at bottom. Shared via share_plus. Button in trip detail screen after accel card. NSPhotoLibraryAddUsageDescription added to ios/Runner/Info.plist for iOS save-to-gallery support.
...
### Known issues
- **Weather icon placeholder** â€” weather card shows `Icons.wb_sunny_outlined` for all weather types; custom painter illustrations planned for polish pass.
...
1. **Custom weather painter illustrations** — replace `Icons.wb_sunny_outlined` placeholder with custom painter illustrations (sun, cloud, rain, thunderstorm, snow)
2. **Animations and transitions** — polish screen and card transitions
3. **Edge cases** — remaining edge case handling across features
## Rules
