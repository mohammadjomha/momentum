...
- `lib/features/auth/screens/verify_email_screen.dart` — blocking email verification screen shown after registration; polls every 4s via `FirebaseAuth.instance.currentUser?.reload()` (re-reads singleton after reload, not stale local ref); on confirmed `emailVerified`, calls `getIdToken(true)` to force token refresh then navigates to `/home`; resend button with 30s debounce; `skipEmailVerification` Firestore flag on `users/{uid}` bypasses screen for existing accounts (checked in `_onMount`)
...
- Email uniqueness enforced natively by Firebase Auth (`createUserWithEmailAndPassword` rejects duplicate emails)
...
- Email verification required after registration: `sendEmailVerification()` called in `auth_service.dart` after successful transaction; new users land on `/verify-email` route and cannot proceed until email is verified; go_router redirect enforces this via `emailVerified` check; `_AuthStateListenable` uses `idTokenChanges()` (superset of `authStateChanges()`) so token refreshes from `getIdToken(true)` trigger router re-evaluation
